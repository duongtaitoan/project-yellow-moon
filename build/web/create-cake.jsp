<%--
    ...
    Created on : Nov 29, 2021, 8:03:47 AM
    Author     : toan
--%>
...
<html>
    ...
    <body>
        ...
        <div class="alertSMS">
            <c:if test="${not empty CREATE_CAKE_MSG_FAILED}">
                <div class="text-warning m-auto alert alert-light alert-dismissible fade show text-center col-5 mt-5 shadow p-3 mb-5 bg-white rounded">
                    <strong>Note:</strong> ${CREATE_CAKE_MSG_FAILED}
                ...
                <div class="wrapper-content">
                    <form method="POST" action="MainController" enctype="multipart/form-data">
                        <div class="form-group mt-3">
                            ...
                            <input type="text" placeholder="Cake name ex: Bánh Trung Thu" value="${txtCakeName}" pattern="[^!@#$%^&*()_+]{0,}" name="txtCakeName" required="" class="form-control">
                        </div>
                        <div class="form-group mt-3">
                            <label>Price</label>
                            <input type="number" min="1000" step="1000" value="${txtPrice}" name="txtPrice" placeholder="Price" required="" class="form-control">
                        </div>
                        <div class="form-group mt-3">
                            ...
                            <select name="selectCategory" class="form-control">
                                ...
                                <c:forEach items="${LIST_CATEGORY}" var="category">
                                    <option value="${category.categoryID}" ${category.getCategoryID() == searchedCategoryID ? 'selected' : ''}>${category.category}</option>
                                </c:forEach>
                            </select>
                        </div>
                        <div class="form-group mt-3">
                        ...
                        <div class="form-group mt-3">
                            ...
                            <input type="text" placeholder="Description ex: Ngon ngọt ít đường nhiều nhân" value="${txtDescription}" pattern="[^!@#$%^&*()_+]{0,}" name="txtDescription" class="form-control">
                        </div>
                        <div class="form-group mt-3">
                            <label>Create Date</label>
                            <input type="date" placeholder="Create Date" value="${txtCreateDate}" name="txtCreateDate"
                                   required=""  max="<%=LocalDate.now().minusDays(1)%>" class="form-control">
                        </div>
                        <div class="form-group mt-3">
                            <label>Expiration Date</label>
                            <input type="date" placeholder="Expiration Date" value="${txtExpDate}" name="txtExpirationDate"
                                   required="" min="<%=LocalDate.now().minusDays(-7)%>" class="form-control">
                        </div>
                        ...
                        <div class="mt-5 btn d-flex justify-content-center">
                            <input class="btn w-75 btn-lg btn-outline-dark" type="submit" name="btnAction" value="Create Cake">
                        </div>
                    </form>
                </div>
            </div>
