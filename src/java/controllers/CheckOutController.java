/*
 * To change this license header, choose License Headers in Project Properties.
 * To change this template file, choose Tools | Templates
 * and open the template in the editor.
 */
package controllers;

import daos.CakeDAO;
import daos.CategoryDAO;
import daos.OrderDAO;
import daos.OrderDetailsDAO;
import define.DateUtils;
import define.Define;
import dtos.CakeDTO;
import dtos.CategoryDTO;
import dtos.OrderDTO;
import dtos.OrderDetailDTO;
import java.io.IOException;
import java.util.ArrayList;
import javax.servlet.ServletException;
import javax.servlet.http.HttpServlet;
import javax.servlet.http.HttpServletRequest;
import javax.servlet.http.HttpServletResponse;
import javax.servlet.http.HttpSession;

/**
 *
 * @author toan
 */
public class CheckOutController extends HttpServlet {

    /**
     * Processes requests for both HTTP <code>GET</code> and <code>POST</code>
     * methods.
     *
     * @param request servlet request
     * @param response servlet response
     * @throws ServletException if a servlet-specific error occurs
     * @throws IOException if an I/O error occurs
     */
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
            throws ServletException, IOException {
        response.setContentType("text/html;charset=UTF-8");
        String url = Define.INDEX_PAGE;
        HttpSession session = request.getSession();
        try {
            ArrayList<CakeDTO> cart = (ArrayList<CakeDTO>) session.getAttribute("CART");
            OrderDAO orderDAO = new OrderDAO();
            OrderDTO orderDTO = new OrderDTO();
            DateUtils dateUtils = new DateUtils();
            CakeDAO cakeDAO = new CakeDAO();

            String payStatus = request.getParameter("paymentStatus");
            String payMethod = request.getParameter("paymentMethod");
            float totalPrice = Float.parseFloat(request.getParameter("txtTotalPrice"));
            String user = (String) session.getAttribute("USER");
            orderDTO.setUserID(user);
            orderDTO.setDate(dateUtils.getCurrentDate());
            orderDTO.setTotalPrice(totalPrice);
            orderDTO.setPaymentMethod(payMethod);

            // set method paypal
            if (payMethod.equalsIgnoreCase("paypal")) {
                orderDTO.setPaymentStatus(payStatus);
                orderDTO.setShippingAddress("");
            } else {
                String address = new String(request.getParameter("txtAddress").getBytes("iso-8859-1"), "UTF-8");
                String phoneNumber = request.getParameter("txtPhoneNumber");
                String fullName = new String(request.getParameter("txtName").getBytes("iso-8859-1"), "UTF-8");
                orderDTO.setFullName(fullName);
                orderDTO.setPhoneNumber(phoneNumber);
                orderDTO.setPaymentStatus(payStatus);
                orderDTO.setShippingAddress(address);
            }

            int insertedOrderID = orderDAO.insertOrder(orderDTO);
            if (insertedOrderID != 0) {
                OrderDetailsDAO orderDetailsDAO = new OrderDetailsDAO();
                for (CakeDTO f : cart) {
                    OrderDetailDTO detailDTO = new OrderDetailDTO();
                    detailDTO.setOrderID(insertedOrderID);
                    detailDTO.setCakeID(f.getCakeID());
                    detailDTO.setQuantity(f.getQuantity());
                    detailDTO.setUnitPrice(f.getPrice());
                    orderDetailsDAO.insertOrderDetail(detailDTO);
                    cakeDAO.subQuantity(f.getCakeID(), f.getQuantity());
                }
                session.removeAttribute("CART");
                request.setAttribute("CHECK_OUT_MSG", "Your order has been accepted");
                if (user != null) {
                    request.setAttribute("ORDER_MSG", "Your order has been accepted\nYou can check order tracking by orderID: (" + insertedOrderID + ")");
                }
            }
            CategoryDAO categoryDAO = new CategoryDAO();
            ArrayList<CategoryDTO> listCategory = categoryDAO.getAll();
            request.setAttribute("LIST_CATEGORY", listCategory);
        } catch (Exception e) {
            log("Error At Check Out Controller " + e.getLocalizedMessage());
//            url = Define.ERROR_PAGE;
        } finally {
            request.getRequestDispatcher(url).forward(request, response);
        }
    }

    // <editor-fold defaultstate="collapsed" desc="HttpServlet methods. Click on the + sign on the left to edit the code.">
    /**
     * Handles the HTTP <code>GET</code> method.
     *
     * @param request servlet request
     * @param response servlet response
     * @throws ServletException if a servlet-specific error occurs
     * @throws IOException if an I/O error occurs
     */
    @Override
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
            throws ServletException, IOException {
        processRequest(request, response);
    }

    /**
     * Handles the HTTP <code>POST</code> method.
     *
     * @param request servlet request
     * @param response servlet response
     * @throws ServletException if a servlet-specific error occurs
     * @throws IOException if an I/O error occurs
     */
    @Override
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
            throws ServletException, IOException {
        processRequest(request, response);
    }

    /**
     * Returns a short description of the servlet.
     *
     * @return a String containing servlet description
     */
    @Override
    public String getServletInfo() {
        return "Short description";
    }// </editor-fold>

}
